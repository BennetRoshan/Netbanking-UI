...
files.forEach(file => {
    ...
    if (file === 'index.html') {
        // Remove the duplicated Nexa Chatbot Bars
        content = content.replace(/<!-- Nexa Chatbot Bar -->[\s\S]*?<!-- Nexa Chat Response Area \(Hidden by default\) -->[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g, '');
...
`;
        if (!content.includes('Nexa Floating Chat Widget')) {
            content = content.replace('</body>', orbHtml + '\n</body>');
        }
    }
