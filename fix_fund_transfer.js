const fs = require('fs');
const path = 'features/transfers/fund-transfer.html';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.DB) {
        ...
        // Do not filter out NEXB here, list all beneficiaries for flexibility.
...
function processTransfer(btn) {
    ...
    const summary = \`
        <div class="d-flex justify-content-between mb-2">
            ...
            <span class="fw-bold small">NEFT/IMPS Transfer</span>
        </div>
    ...
    NexusBank.showOTPModal({
        ...
        onSuccess(txnRef) {
            ...
            const txnData = {
                ...
                desc: 'Fund Transfer to ' + document.getElementById('selectedBeneficiaryText').textContent,
                type: 'Debit',
            ...
            // 3. Credit beneficiary if internal
            ...
            if(benAccountObj) {
                ...
                window.DB.insert('transactions', {
                    ...
                    date: new Date().toISOString().split('T')[0],
                    desc: 'Fund Transfer from ' + (currentUser.name || uid),
                    type: 'Credit',
                    amount: amount.toString(),
                    status: 'Completed',
                    merchant: 'Nexus Bank',
...
console.log("Updated fund-transfer.html");
